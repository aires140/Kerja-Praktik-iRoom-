...
if( isset($_POST["login"]) ) {
  ...
  $result = mysqli_query($conn, "SELECT * FROM tb_akun WHERE nama = '$nama'");
  ...
  if( mysqli_num_rows($result) === 1 ) {
    ...
    if( password_verify($password, $row["password"]) ) {
      ...
      $_SESSION["login"] = true;
      $_SESSION["nama"] = $nama;
      $_SESSION["id_koordinator"] = $row["id_koordinator"];
  ...
  <body> 
    <div class="global-container "> 
      <div class="card login-form position-absolute top-50 start-50 translate-middle shadow">
        <div class="card-body ">
          <h1 class="card-title text-center" style="text-decoration: none;">Hello User</h1>
        </div>     
        <div class="card-text">    
        
        <?php if( isset($error) ) : ?>
          <p>username/password kamu tidak terdaftar</p>
        <?php endif; ?>
        ...
        <div class="create" style="color: #73a0e2;">
          <br>Anda tidak memiliki akun?<br>
          <a href="signup.php" class="text-primary fw-bold">Daftar</a>
        </div>
      </div>
    </div>
